...
else if instance_number(O_rock) < O_game.max_rocks
{
	...
	var max_height = room_height+distance;
	var loc_x=random_range(room_width/2,max_width);
	var loc_y= random_range(room_height/2,max_height);
	effect_create_above(ef_explosion, loc_x, loc_y, 2, c_gray);
	instance_create_layer(loc_x,loc_y, "Instances", O_rock);
    instance_destroy();
}
else
{
